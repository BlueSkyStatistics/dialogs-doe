...
var localization = {
    en: {
		...
		colorThresholdChk: "Highlight a response region by color threshold (optional)",
		colorThresholdPct: "Threshold (% of plotted response range, 1-99)",
		colorThresholdDirectionChk: "Highlight the region ABOVE the threshold (uncheck to highlight BELOW instead)",
		colorThresholdColor: "Highlight color (any R color name)",
		colorThresholdNote: "The contour plot's background color normally uses a continuous gradient across the full plotted response range. When enabled, this splits that gradient at the specified percentage (0% = minimum plotted response, 100% = maximum) and paints the selected side of that split with a single solid highlight color instead — for example, highlighting the top 20% of predicted response (threshold=80%, direction=Above) to visually flag a 'sweet spot' near the optimum. Has no effect unless 'Display Contour(plots)' is also checked.",
        ...
        forceKeepVars: "Force-keep variables as main effects (optional)",
        forceKeepVarsNote: "Stepwise selection runs completely unconstrained, exactly as if this list were empty. Afterward, if any variable selected here is missing from the final model, it is added back as a main effect (linear) term only — its quadratic and interaction terms are not added, since they were not selected on their own merits. If a variable's main effect is already present in the selected model (whether directly significant or retained via hierarchy because one of its interactions/quadratic was selected), this option has no effect on it. Only numeric RSM predictors already present in the formula above can be selected. Has no effect when stepwise selection is disabled.",
...
`
        ...
        var objects = {
			...
			*/
			...
			colorThresholdChk: {
                el: new checkbox(config, {
                    label: localization.en.colorThresholdChk,
			...
			colorThresholdPct: {
                el: new input(config, {
                    ...
                    label: localization.en.colorThresholdPct,
                    placeholder: "e.g., 80",
                    extraction: "TextAsIs",
                    value: "80",
                    required: false,
			...
			colorThresholdDirectionChk: {
                el: new checkbox(config, {
                    label: localization.en.colorThresholdDirectionChk,
			...
			colorThresholdColor: {
                el: new input(config, {
                    ...
                    label: localization.en.colorThresholdColor,
			...
			colorThresholdNote: {
                el: new labelVar(config, {
                    label: localization.en.colorThresholdNote,
            ...
            forceKeepVars: {
                el: new dstVariableList(config, {
                    label: localization.en.forceKeepVars,
                    no: "forceKeepVars",
                    filter: "Numeric|Scale",
            ...
            forceKeepVarsNote: {
                el: new labelVar(config, {
                    label: localization.en.forceKeepVarsNote,
