...
var localization = {
    en: {
        ...
        modelname: "Enter Model Name (dataset name will be appended automatically)",
		
        dependent: "Response (dependent) variable",
        independent: "Independent variable(s)",
		blockID: "If block id to be included in the model",
		...
		showDesignWithoutCenterpointsChk: "Show design data without Center Points (and axial points, if any) used in the linear anlysis",
		
		axialCenterPointRowsChk:"If any, identify the row numbers for center points and axial/star points in the DoE design",
		
		effectsplot:"All effects plot (generated only when Degree = 1 for model without center/axial points)",
       ...
       // weights: "Specify a variable with weights",
		...
		MEPlotRefLineSectionLbl: "Horizontal reference lines (optional)",
		MEPlotyIntercept: "Enter comma separated Y intercept value(s) for the horizontal reference lines, e.g. 150, 166, 185",
		MEPlothorizontalLinelabel: "Specify comma separated labels for the reference lines (no quotes), e.g. LSL, Target, USL",
		
		IAPlotChk: "IAPlot(interaction plots)",
		IAPlotShowAliasChk: "Show aliases in the plot",
		
		showParetoPlotsChk: "Pareto chart of standardized effects", 
		
		observationDiagnosticsTableChk: "Show observation diagnostics table",
		VIFChk: "Show Variance Inflation Factors (VIF)",
...
#require(BsMD)
			   ...
			   cat("Design type detected, if any: ", bsky_design_type_label, "\n")
				...
				{{if(options.selected.showParetoPlotsChk === "TRUE" )}}
						...
						# Matches Minitab's switching rule exactly:
						#   df_error  > 0  -> "Pareto Chart of the Standardized Effects"
						#                     (|t value|, threshold = t(0.975, df_error))
						#   df_error == 0  -> "Pareto Chart of the Effects" (Lenth's PSE)
						#                     (|coef*2|, threshold = Lenth's Margin of Error)
						# The switch is on residual df alone, not an arbitrary cutoff, so an
						# unreplicated / saturated 2-level design (0 residual df) automatically
						# falls back to Lenth's method, exactly as Minitab does.

						bsky_pareto_df_residual <- df.residual(bsky_temp_model_without_block)
						bsky_pareto_raw_coefs   <- coef(bsky_temp_model_without_block)[-1]  # exclude intercept

						if (!is.null(bsky_pareto_df_residual) && bsky_pareto_df_residual > 0) {

							# ── MODE 1: residual df available -> Standardized Effects ──────
							bsky_pareto_threshold <- qt(0.975, bsky_pareto_df_residual)

							bsky_effects_df <- data.frame(
								Term   = names(bsky_pareto_raw_coefs),
							...
							bsky_pareto_ylab     <- "Standardized Effect (|t|)"
							bsky_pareto_title    <- "Pareto Chart of the Standardized Effects"
							bsky_pareto_subtitle <- paste0("Response: ", "{{selected.dependent | safe}}",
														   "   |   Reference line: t(0.975, df=",
														   bsky_pareto_df_residual, ") = ",
														   round(bsky_pareto_threshold, 2))
							bsky_pareto_label    <- paste0("alpha=0.05\n(t=", round(bsky_pareto_threshold, 2), ")")

						} else {

							# ── MODE 2: 0 residual df -> Lenth's PSE (Minitab-matching) ────
							# Effects are twice the coded coefficients (Minitab convention):
							# the effect for a factor is the predicted change in mean
							# response from the low level to the high level.
							bsky_pareto_abs_effects <- abs(bsky_pareto_raw_coefs * 2)

							# Lenth (1989) PSE algorithm
							bsky_pareto_s0  <- 1.5 * median(bsky_pareto_abs_effects)
							bsky_pareto_pse <- 1.5 * median(bsky_pareto_abs_effects[bsky_pareto_abs_effects < 2.5 * bsky_pareto_s0])

							# Margin of Error (ME): t(1-alpha/2, m/3) * PSE, m = number of effects
							bsky_pareto_m   <- length(bsky_pareto_abs_effects)
							bsky_pareto_df_lenth <- bsky_pareto_m / 3
							bsky_pareto_threshold <- qt(0.975, bsky_pareto_df_lenth) * bsky_pareto_pse

							bsky_effects_df <- data.frame(
								Term   = names(bsky_pareto_raw_coefs),
								Effect = bsky_pareto_abs_effects
							)
							bsky_effects_df <- bsky_effects_df[order(bsky_effects_df$Effect, decreasing = TRUE), ]

							bsky_pareto_ylab     <- "Effect"
							bsky_pareto_title    <- "Pareto Chart of the Effects"
							bsky_pareto_subtitle <- paste0("Response: ", "{{selected.dependent | safe}}",
														   "   |   Lenth's PSE = ", round(bsky_pareto_pse, 4))
							bsky_pareto_label    <- round(bsky_pareto_threshold, 2)
						}

						bsky_pareto_plot <- ggplot(bsky_effects_df,
                               aes(x = reorder(Term, Effect), y = Effect)) +
								geom_bar(stat = "identity", fill = "steelblue") +
								geom_hline(yintercept = bsky_pareto_threshold,
										   linetype = "dashed", colour = "red", linewidth = 0.8) +
								annotate("text",
									 x     = nrow(bsky_effects_df) + 0.5,  # top of chart (highest bar position)
									 y     = bsky_pareto_threshold,
									 label = bsky_pareto_label,
									 colour = "red",
									 hjust  = 1.1,
									 vjust  = -0.4,
									 size   = 3) + 
								coord_flip() +
								{{selected.BSkyThemes | safe}} +
								labs(title    = bsky_pareto_title,
									 subtitle = bsky_pareto_subtitle,
									 x        = "Term",
									 y        = bsky_pareto_ylab) 
								
						print(bsky_pareto_plot)
						rm(bsky_pareto_plot, bsky_effects_df, bsky_pareto_threshold, bsky_pareto_df_residual,
						   bsky_pareto_raw_coefs, bsky_pareto_ylab, bsky_pareto_title, bsky_pareto_subtitle,
						   bsky_pareto_label)
						if (exists("bsky_pareto_abs_effects")) rm(bsky_pareto_abs_effects)
						if (exists("bsky_pareto_s0")) rm(bsky_pareto_s0)
						if (exists("bsky_pareto_pse")) rm(bsky_pareto_pse)
						if (exists("bsky_pareto_m")) rm(bsky_pareto_m)
						if (exists("bsky_pareto_df_lenth")) rm(bsky_pareto_df_lenth)
					{{/if}}
        ...
        var objects = {
			...
			*/
			...
			showDesignWithoutCenterpointsChk: {
                el: new checkbox(config, {
                    label: localization.en.showDesignWithoutCenterpointsChk,
			...
			*/
            ...
            MEPlotRefLineSectionLbl: {
                el: new labelVar(config, {
                    label: localization.en.MEPlotRefLineSectionLbl,
                    style: "ml-5",
                    h: 6,
                })
            },
            MEPlotyIntercept: {
                el: new input(config, {
                    no: 'MEPlotyIntercept',
                    label: localization.en.MEPlotyIntercept,
                    placeholder: "",
            ...
            MEPlothorizontalLinelabel: {
                el: new input(config, {
                    ...
                    label: localization.en.MEPlothorizontalLinelabel,
                    placeholder: "",
                    required: false,
                    type: "character",
                    style: "ml-5 mb-2",
			...
			showParetoPlotsChk: {
                el: new checkbox(config, {
                    label: localization.en.showParetoPlotsChk, 
			...
			observationDiagnosticsTableChk: {
                el: new checkbox(config, {
                    label: localization.en.observationDiagnosticsTableChk, 
